...
- **Config / Security / Interceptor / Redis**：统一管理 JWT 鉴权、当前用户上下文、跨域、异常、MyBatis-Plus 分页以及 Redis 内存存储。在 Redis 中托管临时验证码、60秒发送频率锁及 5 次错误锁定计数。
...
flowchart TD
    ...
    Backend --> Redis["Redis (Verification Codes & Rate Limiting)"]

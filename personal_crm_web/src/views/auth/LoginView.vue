  ...
  <div class="page">
    ...
    <section class="auth-col">
      <div class="auth-card">
        ...
        <p class="auth-sub">登录您的 Personal CRM 账户</p>
...
const errorText = ref<string>('账号或密码错误')
...
const handleLogin = async () => {
  ...
  // 2. 原型规定的简易校验拦截 (当账号为 admin 且密码不是 admin 时)
  ...
  try {
    ...
    // 4. 将 API 层的报错显示在原型规定的红框中
    errorText.value = error.message || '账号或密码错误'
    showError.value = true
  } finally {
    loading.value = false
  }
...
const forgotPassword = () => {
  const emailQuery = loginForm.username.includes('@') ? loginForm.username : ''
  router.push({ name: 'forgot-password', query: { email: emailQuery } })
}
...
.ai-avatar {
  ...
  height: 24px;
  border-radius: 6px;
  background: linear-gradient(180deg, #3b5bff, #111827);
  display: flex;
  align-items: center;
  justify-content: center;
  color: #fff;
